...
-	SharedPreferences : sauvegarde les données pour une réutilisation au sein de l'application, utilisé notamment pour la gestion des préférences de l’utilisateur. Exemple : activer ou désactiver la réception de notifications – Inconvénient : ne peut pas être partagé avec d’autres applications ;
